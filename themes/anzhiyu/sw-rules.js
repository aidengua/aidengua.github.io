...
module.exports.config = {
  ...
  serviceWorker: {
    escape: 0,
    cacheName: "dreamersaudioThemeCache",
    debug: false,
  },
  register: {
    onsuccess: undefined,
      ...
      caches.match('https://id.v3/').then(function(response) {
        if (response) {
          ...
          response.json().then(function(data) {
            dreamersaudioPopupManager && dreamersaudioPopupManager.enqueuePopup('通知📢', `已刷新缓存，更新为${data.global + "." + data.local}版本最新内容`, null, 5000);
          });
        } else {
