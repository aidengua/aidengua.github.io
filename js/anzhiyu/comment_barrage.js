if (document.querySelector(".comment-barrage")) {
  ...
  function popCommentBarrage(data) {
    ...
    // 获取dreamersaudio标签内的所有pre元素
    let dreamersaudioPres = barrage.querySelectorAll("anzhiyu pre");

    // 遍历每个pre元素，将其替换为"【代码】"
    dreamersaudioPres.forEach(pre => {
    ...
    // 获取dreamersaudio标签内的所有图片元素
    let dreamersaudioImages = barrage.querySelectorAll("anzhiyu img");

    // 遍历每个图片元素，将其替换为"【图片】"，但排除带有class=tk-owo-emotion的图片
    dreamersaudioImages.forEach(image => {
  ...
  if (localStorage.getItem("commentBarrageSwitch") !== "false") {
    ...
    document.querySelector(".menu-commentBarrage-text").textContent = "關閉熱門評論";
  } else {
    document.querySelector(".comment-barrage").style.display = "none";
    document.querySelector(".menu-commentBarrage-text").textContent = "顯示熱門評論";
  }
